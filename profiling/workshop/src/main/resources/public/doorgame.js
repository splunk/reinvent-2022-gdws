...
function startGame(){
    ...
    show('throbber1');
    hide('start-button');
    document.getElementById('game-heading').innerText = 'TERRAFORMING LABOR DATA ROBOT (TLDR) SYSTEM';
...
function showZonk(){
    document.getElementById('change-prompt').innerHTML = `You picked region #${chosenDoor+1}.<br/>Region #${revealedDoor+1} is not terraformable.<br/>Final decision:`;
    show('change-prompt');
    hide(`door${revealedDoor}`);
    show('change-button');
    document.getElementById('stay-button').innerText = `Stay with region #${chosenDoor+1}`;
    // this is a clever way of finding the remaining door..._too_ clever
    const remaining = 3 - (chosenDoor + revealedDoor);
    document.getElementById('change-button').innerText = `Switch to region #${remaining+1}`;
    show('stay-button');
...
async function finishGame(finalChoice, otherDoor){
    ...
    if(outcome === 'WIN'){
        ...
        show(`zonk${otherDoor}`);
        document.getElementById('win-lose').innerText = "You did it! 🥳";
    }
    else {
        show(`zonk${finalChoice}`);
        show(`money${otherDoor}`);
        document.getElementById('win-lose').innerText = "You chose... poorly. 😒";
    }
    show('win-lose');
    show('play-again');
}
